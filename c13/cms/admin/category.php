...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // Slide 27 코드
  // Part C: 데이터가 유효한지 확인, 유효한다면 데이터베이스 업데이트
  if ($invalid) {
  ...
  // 3. 다른 이유로 예외 발생
  try {
    pdo($pdo, $sql, $argument); // 성공하면 저장
    redirect('categories.php', ['succes' => 'Category saved!']);
